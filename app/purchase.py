...
from app.money import allocate_total, decimal_value
from db.queries import add_expenses, get_expenses
...
class PurchaseTab(ctk.CTkFrame):
    ...
    def add_item(self):
        ...
        try:
            ...
            messagebox.showerror("Invalid Purchase Date", str(exc))
            return

        # Add the item to the current purchases
        item = {"name": name, "component_type": component_type,
                "price": price_after_percentage, "percentage": percentage,
    ...
    def add_bundle(self):
        ...
        items_to_add = list(self.current_purchase_items)
        if self.validate_form():
            ...
            try:
                ...
                messagebox.showerror("Invalid Purchase Date", str(exc))
                return
            items_to_add.append({"name": name, "component_type": item_type,
                                 "price": price_after_percentage,
                                 "percentage": percentage,
                                 "purchase_date": purchase_date})

        percentages = [decimal_value(item.get("percentage", 100), "Bundle percentage")
                       for item in items_to_add]
        if sum(percentages) != 100:
            messagebox.showerror(
                "Invalid Bundle Allocation",
                f"Bundle percentages must total exactly 100% (currently {sum(percentages)}%).",
            )
            return
        total_price = self.price_entry.get().replace(" ", ".")
        allocated_prices = allocate_total(total_price, percentages)
        for item, allocated_price in zip(items_to_add, allocated_prices):
            item["price"] = allocated_price

        try:
            add_expenses(items_to_add)
        except (ValueError, LookupError) as exc:
            messagebox.showerror("Unable to Add Purchase", str(exc))
            return
    ...
    def refresh(self):
        ...
        self.name_entry.set_suggestions(self.get_all_items())
        self.refresh_purchase_listbox()
