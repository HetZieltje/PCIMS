        ...
        if self.var.get() == "":
            ...
            if words:
                if not self.listbox_up:
                    self.listbox = tk.Listbox(self.master, width=min(self.winfo_width(), 50), height=min(len(words), 10), bg="#3e3e3e" if self.master.app.is_dark_mode else "white", fg="white" if self.master.app.is_dark_mode else "black")
